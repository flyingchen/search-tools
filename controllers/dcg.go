...
type DcgController struct {
	BaseController
}

// show the commit file form
func (this *DcgController) Get() {
	v := this.GetString("v")
	if v == "" {
		this.Layout = "layout.html"
		this.TplNames = "dcg.html"
	} else if v == "checkfile" {
...
//接受文件
func receiveFile(this *DcgController) string {
	now := time.Now()
	y, m, d := now.Date()
...
fmt.Println("input file",file,",output file:","./data/out/"+outFile)
